    ...
    <div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="bg-[#1C1C1E] border border-white/10 rounded-2xl shadow-2xl w-full max-w-md mx-4 p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-[16px] font-semibold text-white">New Portfolio</h3>
            ...
            <div className="space-y-3">
              <input
                ...
                className="w-full px-3 py-2.5 text-sm bg-white/5 border border-white/10 rounded-lg focus:ring-1 focus:ring-[#6B7A2A] outline-none text-white placeholder:text-gray-600"
                autoFocus
              />
              <textarea
                value={description}
                ...
                <div className="flex gap-2">
                  {COLORS.map((c) => (
                    <button
                      key={c}
                      onClick={() => setColor(c)}
                      className={`w-7 h-7 rounded-full transition ${color === c ? "ring-2 ring-offset-2 ring-offset-[#1C1C1E] ring-white/60" : ""}`}
              ...
              <div className="flex gap-2 pt-2">
                ...
                <button onClick={() => setShowCreate(false)} className="px-4 py-2.5 text-sm text-gray-400 hover:text-white">
                  Cancel
                </button>
              </div>
            </div>
      ...
      {portfolios.length === 0 ? (
        <div className="text-center py-14 bg-[#1C1C1E] rounded-xl border border-white/8">
          ...
          <p className="text-gray-400 mb-4">No portfolios yet. Group your projects together!</p>
          <button
            onClick={() => setShowCreate(true)}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {portfolios.map((portfolio) => (
            <div key={portfolio.id} className="bg-[#1C1C1E] rounded-xl border border-white/8 p-5 group">
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    {portfolio.description && <p className="text-sm text-gray-400 mt-0.5">{portfolio.description}</p>}
                    <p className="text-xs text-gray-500 mt-1">{portfolio.team.name} · {portfolio.projects.length} projects</p>
                  </div>
                </div>
                <button
                  onClick={() => handleDelete(portfolio.id)}
                  className="p-1.5 text-gray-500 hover:text-red-400 rounded-lg hover:bg-red-500/10 transition opacity-0 group-hover:opacity-100"
                >
                ...
                <div className="space-y-2">
                  {portfolio.projects.map((pp) => (
                    ...
                    >
                      ...
                      <span className="text-sm text-gray-300 flex-1">{pp.project.name}</span>
                      <span className="text-xs text-gray-500">{pp.project._count.tasks} tasks</span>
                    </Link>
                  ))}
                </div>
              ) : (
                <div className="text-center py-4 text-sm text-gray-500 bg-white/5 rounded-lg">
